...
_CANDIDATES = {
    ...
    "left_from": ["LFROMADD", "L_FROM_ADD", "LEFT_FROM", "LEFTFROM", "L_FROM"],
    "left_to": ["LTOADD", "L_TO_ADD", "LEFT_TO", "LEFTTO", "L_TO"],
    "right_from": ["RFROMADD", "R_FROM_ADD", "RIGHT_FROM", "RIGHTFROM", "R_FROM"],
    "right_to": ["RTOADD", "R_TO_ADD", "RIGHT_TO", "RIGHTTO", "R_TO"],
...
def _state_from_row(row: pd.Series, state_column: str | None, configured: str | None) -> str:
    ...
    # STATEFP is numeric in many TIGER tables. It is not suitable for blocking
    # against the usual USPS abbreviation, so retain it as a stable local code
    ...
    for row_number, (original, projected_row) in enumerate(
        ...
        for id_col in ("TLID", "LINEARID", "RANGE_ID", "ID"):
            ...
            if id_col in matches:
                ...
                break
        offset = min(max(float(config.end_offset_m), 0.0), line.length * 0.49)
        ...
        for side, start_col, end_col, zip_col, city_col in (
            ...
            for house_number in numbers:
                ...
                point_projected = _offset_point(line, distance, side, float(config.side_offset_m))
                ...
                output_rows.append(
                    {
                        ...
                        "street_block": normalize_text(street_name)[:1],
                        "city": city,
                        "city_norm": city,
                        "state": state_value,
